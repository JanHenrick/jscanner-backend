...
# ── FEATURE 2: Image Direct Convert ──────────────────────────────────
@app.post("/image/pdf")
async def image_to_pdf(file: UploadFile = File(...)):
    contents = await file.read()
    from services.image_convert import image_to_pdf
    output_path = image_to_pdf(contents, file.filename)
    return FileResponse(output_path, media_type="application/pdf", filename="image.pdf")

@app.post("/image/word")
async def image_to_word(file: UploadFile = File(...)):
    contents = await file.read()
    from services.image_convert import image_to_word
    output_path = image_to_word(contents, file.filename)
    return FileResponse(output_path,
        media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        filename="image.docx")

@app.post("/image/excel")
async def image_to_excel(file: UploadFile = File(...)):
    contents = await file.read()
    from services.image_convert import image_to_excel
    output_path = image_to_excel(contents, file.filename)
    return FileResponse(output_path,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        filename="image.xlsx")